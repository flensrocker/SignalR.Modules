...
namespace SignalR.Modules.Client
{
    ...
    public static partial class ModuleHubClientExtensions
    {
        ...
        public static IDisposable On(this ModuleHubClient hubConnection, string methodName, Action handler)
        {
        ...
        /// <param name="handler">The handler that will be raised when the hub method is invoked.</param>
        /// <returns>A subscription that can be disposed to unsubscribe from the hub method.</returns>
        public static IDisposable On(this ModuleHubClient hubConnection, string methodName, Func<Task> handler)
        {
            if (hubConnection == null)
            {
                throw new ArgumentNullException(nameof(hubConnection));
